...
if __name__ == "__main__":
    ...
    with h5py.File(dataset_path, 'r') as f:
        ...
        # ✓ Garder 5D pour MultiTypeTemporalCorrelationCNN: (B, n_types, n_deltas, H, W)
        # Le dataset est (B, n_deltas, n_types, H, W) → permuter en (B, n_types, n_deltas, H, W)
        X = X.permute(0, 2, 1, 3, 4).contiguous()
        print(f"  Images shape après permute (5D): {X.shape}")

        print(f"  ✓ Targets normalisés")
        print(f"  Range après normalisation: [{y.min():.3f}, {y.max():.3f}]")

        # Ne pas aplatir en 4D pour ce modèle
        # X = X.reshape(...)  # ← supprimé
    ...
    for i, p in enumerate(params):
        ...
        # Créer un nouveau modèle pour chaque config (auto à partir des données)
        ...
        trained_model, optimizer, metrics = train_model(
            name="MultiTypeTemporalCorrelationCNN-wo-dropout-ds5-normalized-R2-precip-dd",
            model=model,
            dataset=dataset,
            num_epochs=p[0],
            batch_size=p[1],
        ...
        save_model_checkpoint(
            model=trained_model,
            optimizer=optimizer,
            model_name="MultiTypeTemporalCorrelationCNN-wo-dropout-ds5-normalized-R2-precip-dd",
            epoch=metrics['epoch'],
